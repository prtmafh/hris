...
@section('content')
    <main>
                            ...
                            <select name="kategori_reimbursement_id" class="form-select form-select-sm">
                                ...
                                @foreach ($kategoriList as $kategori)
                                    <option value="{{ $kategori->id }}"
                                        {{ request('kategori_reimbursement_id') == $kategori->id ? 'selected' : '' }}>
                                        {{ $kategori->nama }}
                                    </option>
                                @endforeach
                            </select>
                                        ...
                                        <td class="text-center">
                                            ...
                                            @if ($item->buktiFile->isNotEmpty())
                                                <div class="dropdown d-inline">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        ...
                                                        @foreach ($item->buktiFile as $bIndex => $bukti)
                                                            <li>
                                            ...
                                            @if ($item->status === 'pending')
                                                <button
                                                    ...
                                                    data-bs-target="#modalApprove{{ $item->id }}" title="Setujui">
                                                    <i data-feather="check"></i>
                                                </button>
                                                <button
                                                    class="btn btn-datatable btn-icon btn-transparent-dark text-danger me-1"
                                            ...
                                            @if ($item->status === 'disetujui')
                                                <form action="{{ route('admin.reimbursement.bayar', $item->id) }}"
                                                    method="POST" class="d-inline"
                                                    id="formTandaiBayar{{ $item->id }}">
                                                    @csrf
                                                    <button
                                                        class="btn btn-datatable btn-icon btn-transparent-dark text-primary me-1"
                                                        onclick="confirmTandaiBayar(event, {{ $item->id }})"
                                            ...
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('admin.reimbursement.destroy', $item->id) }}"
                                                method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                            </form>
        ...
        {{--
        =====================================================================
        SEMUA MODAL PER-ITEM DITARUH DI SINI, DI LUAR <table> DAN DI LUAR
        .table-responsive.

        Kenapa dipindah:
        Sebelumnya modal-modal ini ditaruh di dalam <tbody>, yang secara HTML
        tidak valid (tbody cuma boleh berisi <tr>), dan juga masih terjebak
        di dalam .table-responsive yang punya overflow-x: auto.

        Bootstrap modal butuh position: fixed relatif ke VIEWPORT. Browser
        WebKit (Safari & semua browser di iOS termasuk Chrome iOS) punya bug
        lama: elemen fixed di dalam parent yang overflow:auto/scroll gagal
        "lepas" ke viewport, sehingga modal jadi terpotong / salah posisi /
        cuma sebagian yang muncul, sementara backdrop tetap menutupi &
        memblokir klik ke halaman.

        Chrome/Android (Blink) lebih toleran soal ini sehingga modal tetap
        muncul normal di sana walau HTML-nya sebenarnya invalid.

        Dengan modal dipindah ke sini (sejajar dengan modalTambah, di luar
        semua container yang overflow), modal akan center dengan benar di
        semua browser, termasuk WebKit/iOS.
        =====================================================================
    --}}
            ...
            <div class="modal-content">
                <form method="POST" action="{{ route('admin.reimbursement.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <textarea name="deskripsi" class="form-control mb-3" rows="3" placeholder="Deskripsi (opsional)"></textarea>
...
@push('scripts')
    <script>
        function confirmTandaiBayar(event, id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('formTandaiBayar' + id).submit();
                }
            });

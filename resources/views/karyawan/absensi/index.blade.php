    ...
    <div class="container-xl px-4">
        ...
        <nav class="nav nav-borders mb-4">
            <a class="nav-link {{ request('type') !== 'sesi' ? 'active' : '' }}"
                href="{{ route('data_absenKaryawan', array_merge(request()->query(), ['type' => 'biasa'])) }}">
                <i data-feather="clock" class="me-2" style="width: 16px;"></i>
                Absensi Biasa
            </a>
            <a class="nav-link {{ request('type') === 'sesi' ? 'active' : '' }}"
                href="{{ route('data_absenKaryawan', array_merge(request()->query(), ['type' => 'sesi'])) }}">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route('data_absenKaryawan') }}" class="row gx-2 gy-2 align-items-end">
                    ...
                    <div class="col-md-2 d-flex gap-1">
                        ...
                        <a href="{{ route('data_absenKaryawan') }}" class="btn btn-light btn-sm">
                            <i data-feather="x"></i>
                        </a>
                    </div>
                </form>
                    ...
                    <table id="datatablesSimple" class="table responsive">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('karyawan.absensi-sesi.show', $sesi->id) }}"
                                        class="btn btn-datatable btn-icon btn-transparent-dark me-2">
                                        <i data-feather="eye"></i>
                                    </a>
                                    <a href="{{ route('karyawan.absensi-sesi.edit', $sesi->id) }}"
                                        class="btn btn-datatable btn-icon btn-transparent-dark me-2">
                                    ...
                                    <form id="delete-form-{{ $sesi->id }}"
                                        action="{{ route('karyawan.absensi-sesi.destroy', $sesi->id) }}" method="POST"
                                        style="display:none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                            ...
                            <tr>
                                ...
                                {{-- AKSI --}}
                                <td>
                                    <a href="{{ route('karyawan.absensi.show', $a->id) }}"
                                        class="btn btn-datatable btn-icon btn-transparent-dark me-2">
                                        <i data-feather="eye"></i>
                                    </a>
                                    <a href="{{ route('karyawan.absensi.edit', $a->id) }}"
                                        class="btn btn-datatable btn-icon btn-transparent-dark me-2">
                                        <i data-feather="edit"></i>
                                    </a>
                                    <button class="btn btn-datatable btn-icon btn-transparent-dark text-danger"
                                    ...
                                    <form id="delete-form-{{ $a->id }}"
                                        action="{{ route('karyawan.absensi.destroy', $a->id) }}" method="POST"
                                        style="display:none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
